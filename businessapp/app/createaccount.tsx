...
export default function CreateAccountScreen() {
  ...
  const handleCreate = async () => {
    ...
    if (requiresEmailConfirmation) {
      Alert.alert('Account created', 'Please verify your email, then sign in.', [{ text: 'Go to Sign In', onPress: () => router.replace('/signin') }]);
      return;
    }
  ...
  const handleOAuth = async (provider: 'google' | 'facebook') => {
    ...
    if (error) { setSocialLoading(null); Alert.alert(`${provider} sign up failed`, error.message); return; }
    const { complete } = await isMerchantSetupComplete();
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <KeyboardAwareScrollView enableOnAndroid={true} extraScrollHeight={80} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
